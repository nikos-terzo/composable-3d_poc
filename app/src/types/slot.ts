export type MoveSlotField = {
    id: {
        id: string
    },
    name: {
        type: `${string}::object3d::SlotID`,
        fields: { id: number }
    },
    value: {
        type: `${string}::object3d::Slot`,
        fields: MoveSlot
    }
};
...
export function intoSlot(moveSlot: MoveSlot): Slot {
    const transform = moveSlot.transform.fields;
    const denominator = parseInt(transform.denominator);
    return {
        attached: moveSlot.attached,
        id: moveSlot.id.fields.id,
        transform: {
